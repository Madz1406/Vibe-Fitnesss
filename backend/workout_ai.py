...
class WorkoutAIGenerator:
    ...
    def generate_workout_plan(self, user_profile: Dict[str, Any], days: int = 7) -> Dict[str, Any]:
        ...
        return {
            ...
            "duration": "8 weeks",
            "frequency": len(workout_schedule),
    ...
    def _generate_schedule(self, template: Dict[str, Any], user_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        days_available = user_profile.get('daysAvailable')
        
        schedule = {}
        days_list = list(days.items())
        
        # Adjust days_list if daysAvailable is provided
        if days_available:
            try:
                target_days = int(days_available)
                if target_days < len(days_list):
                    days_list = days_list[:target_days]
                elif target_days > len(days_list):
                    extra_needed = target_days - len(days_list)
                    extra_days = [
                        ("Sunday", {"name": "Active Recovery / Cardio", "exercises": [{"name": "Light Jogging or Walking", "duration": "30 mins"}], "notes": "Keep heart rate moderate."}),
                        ("Wednesday" if "Wednesday" not in days else "Friday", {"name": "Core & Mobility", "exercises": [{"name": "Yoga / Stretching", "duration": "30 mins"}], "notes": "Focus on flexibility."})
                    ]
                    added = 0
                    for ed in extra_days:
                        if added < extra_needed and ed[0] not in [d[0] for d in days_list]:
                            days_list.append(ed)
                            added += 1
                    
                    # Fill remaining with generic active days
                    day_names = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
                    idx = 0
                    while added < extra_needed:
                        d_name = day_names[idx % 7]
                        if d_name not in [d[0] for d in days_list]:
                            days_list.append((d_name, {"name": "Full Body Light", "exercises": [{"name": "Bodyweight Circuit", "duration": "20 mins"}], "notes": "Extra volume."}))
                            added += 1
                        idx += 1
            except (ValueError, TypeError):
                pass
                
        for day_name, day_data in days_list:
            schedule[day_name] = {
                "name": day_data.get('name', day_name),
                "type": day_data.get('type', 'mixed'),
                "duration": day_data.get('duration', "60 mins"),
                "exercises": day_data.get('exercises', []),
                "notes": day_data.get('notes', f"Focus on controlled movements. Rest 2-3 minutes between sets.")
            }
