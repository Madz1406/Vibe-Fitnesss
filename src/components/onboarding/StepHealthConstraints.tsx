    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3">
        {conditions.map((condition, index) => (
          <div key={condition.id} className="animate-fade-up opacity-0" style={{ animationDelay: `${index * 100}ms`, animationFillMode: 'forwards' }}>
            <button
              onClick={() => toggleCondition(condition.id)}
              className={`w-full p-4 rounded-lg border-2 transition-all text-left ${medicalConditions.includes(condition.id)
                ? 'border-neon-fuchsia bg-neon-fuchsia/10 text-slate-900 dark:text-white'
                : 'border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 hover:border-neon-fuchsia dark:hover:border-slate-500 shadow-sm dark:shadow-none'
                }`}
            >
      ...
      <div className="text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-600 pt-4">
